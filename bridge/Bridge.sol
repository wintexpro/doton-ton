...
contract Bridge {
    ...
    function relayerVoteForProposal(uint8 choice, uint8 chainId, uint64 nonce, bytes32 data, uint256 relayerPubKey) isValidRelayer(relayerPubKey) external {
        require(msg.value >= 400000000); // TODO ???
        IBridgeVoteController(voteControllerAddress).voteByBridge{bounce:true, value:200000000}(msg.sender, choice, chainId, nonce, data);
    }
}